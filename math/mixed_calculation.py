...
def create_single_page(doc, page_number):
    """在文档中添加一页内容"""
    # 添加标题
    title = doc.add_heading(f'加减乘除混合练习题（第{page_number}页）', 0)
...
def create_word_document_with_pages(num_pages=10):
    ...
    # 添加总标题
    main_title = doc.add_heading('加减乘除混合练习题集', 0)
    ...
    # 保存文档
    filename = f'加减乘除混合练习题_{num_pages}页.docx'
    doc.save(filename)
    print(f"\n✅ 加减乘除混合练习题已生成并保存为 '{filename}'")
    print(f"文档包含{num_pages}页，每页100道题目（25道加法 + 25道减法 + 25道乘法 + 25道除法，顺序随机）")
    return filename
